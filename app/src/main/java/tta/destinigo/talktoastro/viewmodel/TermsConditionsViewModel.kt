...
class TermsConditionsViewModel  constructor(app: tta.destinigo.talktoastro.BaseApplication): BaseViewModel(app) {
    ...
    private fun getTermsAndConditions() {
        doApiRequest(
            RequestType.GET, tta.destinigo.talktoastro.volley.RequestIdentifier.TERMSANDCONDITIONS.ordinal,
            ApplicationConstant.TERMS_CONDITIONS, null, null, null, false)
    }
    ...
    override fun onApiResponse(identifier: Int, response: JSONObject, serverDate: Long, lastModified: Long) {
        ...
        if(identifier == tta.destinigo.talktoastro.volley.RequestIdentifier.TERMSANDCONDITIONS.ordinal) {
