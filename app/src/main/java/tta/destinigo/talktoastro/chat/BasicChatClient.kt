    ...
    , AnkoLogger
{
    ...
    override fun onError(errorInfo: ErrorInfo?) {
        BaseApplication.instance.logErrorInfo("Login error", errorInfo!!)
        chatClient = null
    ...
    override fun onTokenAboutToExpire() {
        if (chatClient != null) {
            BaseApplication.instance.showToast("Token will expire in 3 minutes. Getting new token.")
            getAccessToken()
        }
    }
    ...
    override fun onTokenExpired() {
        accessToken = null
        if (chatClient != null) {
            BaseApplication.instance.showToast("Token expired. Getting new token.")
            getAccessToken()
        }
    }
